...
func getOrderDetails(OrderId int) Order_Return {
	...
	fmt.Println("kkk", returnDate)

	userSQL := `SELECT id,student_id,return_date FROM "public"."order"  WHERE id=$1`

	rows := DB.QueryRow(userSQL, OrderId)

	err := rows.Scan(&Data.OrderId, &Data.Student_id, &returnDate)
	//fmt.Println("dipttt", Data)
...
func UpdateFine(Data Order_Return) bool {
	...
	layout := "2006-01-02"
	t22, _ := time.Parse(layout, today)
	userSQL := `UPDATE "order" SET  actual_return_date=$1, fine=$2, return_status=$3 WHERE id=$4`

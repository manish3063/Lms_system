...
func Order_approve(c *gin.Context) {
	reqBody := Order{}
	err := c.Bind(&reqBody)
	//fmt.Println("after binding", re)
	fmt.Println("newerror", reqBody)
	fmt.Println("aasd", reqBody.Request_id)
	...
	result := approve_action(reqBody.Request_id, reqBody.Approved)
...
func approve_action(request_id int, approve string) bool {
	...
	sqlStatement := `INSERT INTO "public"."order" ("book_id", "order_request_id", "student_id", "issue_date", "return_date", "approved")
	VALUES ($1, $2, $3, $4, $5, $6);`
